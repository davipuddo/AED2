...
bool swapAbilities (Pokemon* px, Pokemon* py)
{
	bool res = false;
	if (px && py)
	{
		char* x = px->abilities->data[0];
		char* y = py->abilities->data[0];
...
int getMax (Pokemon** pokes, int n)
{
	int max = 0;
	for (int i = 0; i < n; i++)
	{
		int tmp = (int)strlen(pokes[i]->abilities->data[0]);
		if (max < tmp)
		{
			max = tmp;
		}
...
// Ordenar
void ReSort (Pokemon** pokes, int n)
{
	for (int i = 1; i < n; i++)
	{
		Pokemon* tmp = pokes[i];
		...
		while (y >= 0 && swapAbilities(pokes[y], tmp))
		{
			pokes[y+1] = pokes[y];
			y--;
			mv++;
...
void sort (Pokemon** pokes, int n, int p)
{
	for (int pos = p-1; p >= 0; p--)
	{
		int aux[256] = {0};
		...
		for (int i = 0; i < n; i++)
		{
			int c = getChar(pokes[i]->abilities->data[0], pos);
			aux[c]++;
			cmp++;
		}
		cmp++;
