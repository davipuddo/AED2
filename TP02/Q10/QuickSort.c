...
bool swapGen (Pokemon* px, Pokemon* py)
{
	bool res = false;
...
void Qsort (Pokemon** pokes, int L, int R)
{
	...
	int pivo = pokes[tmp]->generation;
	int i = L;
	int y = R;

	while (i <= y)
	{
		while (pokes[i]->generation < pivo)
		{
			i++;
			cmp++;
		}
		while (pokes[y]->generation > pivo)
		{
			y--;
			cmp++;
		}
...
int main (void)
{
	...
	if (list)
	{
		...
		{	
			...
			printStats("853355_quicksort.txt");
		}
	}
}
