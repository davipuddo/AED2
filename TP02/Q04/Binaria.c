...
bool searchB (Pokemon** pokes, int L, int R, char* x)
{
	bool res = false;
	...
	else if (strcmp (pokes[M]->name, x) == 0)
	{
		res = true;
		cmp++;
	}
	else if (strcmp (pokes[M]->name, x) > 0)	// Parte de baixo
	{
		res = searchB (pokes, L, M-1, x);
		cmp += 2;
	}
	else										// Parte de cima
	{
		res = searchB (pokes, M+1, R, x);
		cmp += 2;
	}
	cmp++;
...
// Encapsular pesquisa
bool search (Pokemon** pokes, int n, char* name)
{
	bool res = false;
	if (pokes && name)
	{
		res = searchB (pokes, 0, n-1, name);
	}
	cmp++;
	return (res);
}
...
int main (void)
{
	...
	if (list)
	{
		...
		{	
			...
			for (i = 0; i < names->n; i++)
			{
				res = search (pokes, Psize, names->data[i]);
