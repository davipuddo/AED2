...
class Pokedex 
{
	...
	public Pokedex ()
	{
		this.data = new Pokemon [30];
	}

	private int hashCode (String name, int x)
	{
		int res = 0;
		int size = 21;
	...
	{
		return (hashCode(name, 0));
	}

	private int ReHashCode (String name)
	{
		return (hashCode(name, 1));
	}

	private void insert (Pokemon poke)
	{
		if (poke != null)
		{
			int pos = hashCode(poke.getName());
			
			if (this.data[pos] == null)
			{
				this.data[pos] = poke.clone();
			}
			else
			{
				pos = ReHashCode(poke.getName());
	...
	public boolean search (String name)
	{
		...
		if (name != null)
		{
			...
			if (this.data[pos] != null)
			{
				if (!this.data[pos].getName().equals(name))	
				{
					// Procurar com ReHash
					pos = ReHashCode(name);

					if (this.data[pos] != null && this.data[pos].getName().equals(name))
					{
						res = true;
					}
					TP.cmp += 2;
